  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/80 flex items-center justify-center p-4 backdrop-blur-md">
      <div className="bg-white rounded-3xl w-full max-w-md p-8 border-4 border-black relative animate-in zoom-in duration-200 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]">
        ...
        <p className="text-[10px] font-bold text-gray-500 mb-6 uppercase">Tạo tài khoản tại CHIN SHOP</p>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-3 bg-gray-100 p-2 rounded-xl border-2 border-black">
            <div className="bg-black text-cyan-400 px-4 py-2 font-mono text-xl italic select-none">{captchaCode}</div>
            <button type="button" onClick={generateCaptcha} className="text-xl">🔄</button>
            <input type="text" placeholder="Mã..." required value={inputCaptcha} onChange={(e) => setInputCaptcha(e.target.value)} className="w-full max-w-[120px] p-2 rounded-lg border-2 border-black uppercase font-black outline-none focus:bg-yellow-50 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" />
          </div>
          <button type="submit" className="w-full bg-red-600 text-white font-black py-4 rounded-2xl text-sm uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all border-2 border-black">
            🚀 XÁC NHẬN ĐĂNG KÝ
          </button>
        </form>
      </div>
